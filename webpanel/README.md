...
**Phạm vi bản v1:** trạng thái, start/stop/restart, xem log realtime
(journalctl), import verification key, xem AccessControlVerificationKeyRing +
traffic-rules.json hiện tại. Các thao tác nhạy cảm/phá hoại (generate signing
keypair mới sẽ vô hiệu hoá toàn bộ token cũ, cấu hình protocol/port, gỡ cài
đặt...) tạm để lại ở CLI — nói mình biết nếu bạn muốn đưa thêm lên web.
